...
export function BoardPage() {
  ...
  const { canEditIssue, canManageSprints, isAdmin } = usePermissions(projectId)
        ...
        {STATUS_COLUMNS.map((status) => (
          <article key={status} className={`kanban-col${dropStatus === status ? ' kanban-col-drop-active' : ''}`} onDragOver={(event) => { event.preventDefault(); if (dropStatus !== status) setDropStatus(status) }} onDrop={() => handleDrop(status)}>
            ...
            {grouped[status]?.map((issue) => (
              <div className="card kanban-card-draggable" key={issue.id} draggable={canEditIssue} onDragStart={() => { if (canEditIssue) setDragIssueId(issue.id) }} onDragEnd={() => { setDragIssueId(null); setDropStatus('') }}>
                <button className="issue-link" type="button" onClick={() => navigate(`/issues/${issue.id}`)}>{issue.key}</button>
                <h4>{issue.title}</h4>
                <p>{issue.issueType}</p>
                <select value={issue.status} onChange={(event) => handleMove(issue.id, event.target.value, issue.sprintId ?? null)} disabled={!canEditIssue}>
