...
describe('ProjectTopPanel — hidden on Users page (JL-374)', () => {
  it('does not render the navigation strip at /users', () => {
    renderAt('/users')
  ...
  it('does not render the navigation strip at a nested sub-path /users/5', () => {
    renderAt('/users/5')
    expect(screen.queryByRole('navigation', { name: /project views/i })).not.toBeInTheDocument()
    expect(screen.queryByRole('link', { name: /backlog/i })).not.toBeInTheDocument()
  })
  ...
  it('remains hidden on /dashboard and /profile (regression control)', () => {
