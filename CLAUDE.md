...
- **Design system (JL-437 → JL-442):** the visual language lives in exactly **three** files and nowhere else:
  1. `src/styles/variables.css` — every value. Colour, type scale, spacing, radius (`--radius-xs|sm|md|lg|pill` = 3/6/8/12/999), shadow, and the layout/control geometry (`--layout-sidebar-width` 288px, `--layout-header-height` 64px, `--layout-breadcrumb-height` 56px, `--layout-details-panel-width` 304px, `--layout-content-padding` 24px, `--control-height` 40px plus the `-sm|flag|status|nav|search|create` variants, `--avatar-size-md|lg` = 32/40).
  ...
  **The issue-detail page is the visual reference.** Its title is the one consumer of `--font-size-display` (30/700/36); a generic page title stays on `.page h1` (24px), because a page title and an issue summary are different roles. Breadcrumbs used to exist twice (`.project-breadcrumb-*` and `.id-breadcrumb-*`, drifted on separator colour and current-item weight) and are now one shared family.

- **Density (JL-442) — read this before changing any size.** JL-438 shifted the whole type ladder up one step in one edit, and JL-439 promoted the issue-detail page's own chrome (a 54px Create button, a 52px search field, a 48px sidebar row, a 36px title, an 18px `--font-size-detail`) to the *global* contract. Every guard still passed, because every call site was still spelling a token — but 58 rules written when `md` meant 16px silently began rendering 20px, and across 44 pages the app read as oversized. JL-442 is the correction:
  - **The ladder is 12 / 14 / 16 / 18 / 20 / 24 / 28**, plus 30 for the two display roles (`--font-size-brand` wordmark, `--font-size-display` issue title). The body end (`xs`/`sm`/`base`) is untouched; `md`/`lg`/`xl`/`xxl` each dropped one step and their leadings moved onto Atlassian's real pairings (20/24, 24/28, 28/32). **Nothing in normal UI reaches 36–40px.**
  - **`--font-size-detail` (18px) and `--space-28` are gone.** `detail` carried section headings, breadcrumbs *and* field labels at one size; those split onto `base` (16) and `sm` (14), which already existed. Do not reintroduce either.
  - **One control scale: 40px.** `--control-height-flag|status|nav|search|create` were 42/44/48/52/54 and are now 36/40/40/40/40. A page that wants a 48px control is asking for a second scale.
  - **Roles:** section heading 16/600, nav 16/400, breadcrumb 16/400 (current 600), body 16/400, button 14/500, table 14/400 in 8×12 cells, detail label 14/400, detail value 16/400, modal title 20, page title 24, issue title 30/700/36. `--icon-md` is 18px.
  - `src/test/UiDensity.JL442.test.js` pins all of it — the ladder values, the leading pairings, the 40px control ceiling, the avatar ranges, the shell-chrome ceilings, and a rule that no shared control class (`.btn`, `.nav`, `.tab`, `.badge`, `.breadcrumb-*`, `.table`, …) may ask for a step above 20px. The earlier guards check *consistency*; this one checks the *values*, which is the failure mode none of them could catch.
  - The font stack now leads with `"Atlassian Sans"` as requested. It is proprietary and not shipped, so it does not resolve on most machines — which is why `'Inter Variable'` stays second and remains the face that actually renders. Do not drop Inter to "simplify" the stack.
